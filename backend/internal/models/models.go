...
type Author struct {
	...
	Books []*Book `bun:"m2m:book_to_author,join:Author=Book"`
}
...
type Book struct {
	...
	Authors    []*Author   `bun:"m2m:book_to_author,join:Book=Author"`
...
type BookToAuthor struct {
	bun.BaseModel `bun:"table:book_to_author"`

	BookID   uuid.UUID `bun:"book_id,pk,type:uuid"`
	AuthorID uuid.UUID `bun:"author_id,pk,type:uuid"`
